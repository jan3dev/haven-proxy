...
function ensureFetchWrapper() {
  ...
  globalThis.fetch = async (input, init) => {
    ...
    // shared attestation/HPKE-keys fetches. The SecureClient SDK dedupes
    // attestation across concurrent calls on one client (one `ready()` /
    // `initPromise`); aborting that shared fetch on one call's timeout tears
    // down init for every other in-flight call (and trips the SDK's
    // transient-error retry). A hang in
